$(document).ready(function () {
	...
	// Son custom - 16/09/2019
	$('.nama-canhan-4 .item a figure .img').each(function () {
	...
	$('.nama-doanhnghiep-2 .item a figure .img').each(function () {
		$(this).height($(this).width())
		$(this).find('img').each(function () {
			if ($(this).width() < $(this).height()) {
				$(this).css({
	...
	if ($('.Module-186').length > 0) {
		if ($(window).scrollTop() >= $('.nama-breadcrumb').height()) {
			$('.nama-navigation-1').addClass('scroll-fixed')
		} else {
			$('.nama-navigation-1').removeClass('scroll-fixed')
		}
	}
	if ($('.Module-188').length > 0) {
		if ($(window).scrollTop() >= $('.nama-breadcrumb').height()) {
			$('.nama-navigation-1').addClass('scroll-fixed')
		} else {
			$('.nama-navigation-1').removeClass('scroll-fixed')
	...
	MonthSlider();
	MoneySlider();
	// END Custom 16/09/2019
});


function MonthSlider() {
	let input = $("#month");
	let minVal = parseInt($("#month").attr("min"))
	let maxVal = parseInt($("#month").attr("max"))
	let step = parseInt($("#month").attr("step"))
	let slider = $("<div id='month-slider'></div>").insertAfter(input).slider({
		min: minVal,
		max: maxVal,
		range: step,
...
function MoneySlider() {
	let input = $("#money");
	let minVal = parseInt($("#money").attr("min"))
	let maxVal = parseInt($("#money").attr("max"))
	let step = parseInt($("#month").attr("step"))
	let slider = $("<div id='money-slider'></div>").appendTo($(".money-slider")).slider({
		min: minVal,
		max: maxVal,
		range: step,
		step: 1000000,
